...
export class BookingService {
  ...
  async attemptBooking(config, mockSlots = null) {
    ...
    if (mockSlots !== null) {
      ...
      availableSlots = await this.scraper.scrape(config.booking_url, preferences);
    } else {
      throw new Error('No slots provided and no scraper configured');
    }

    // From here, the scraper browser may be open — ensure cleanup in all paths
    try {
      ...
      // Always close the browser when scraper was used
